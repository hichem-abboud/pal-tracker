...
public class JdbcTimeEntryRepository implements TimeEntryRepository {
    ...
    public TimeEntry find(long timeEntryId) {
        TimeEntry timeEntry = jdbcTemplate.query("SELECT id, project_id, user_id, date, hours FROM time_entries WHERE id = ?",new Object[]{timeEntryId},resultSetExtractor);
        return timeEntry;
    }
    RowMapper<TimeEntry> mapper = (rs, rowNum) -> {
    ...
    ResultSetExtractor<TimeEntry> resultSetExtractor = rs -> rs.next()?mapper.mapRow(rs,1):null;
    @Override
